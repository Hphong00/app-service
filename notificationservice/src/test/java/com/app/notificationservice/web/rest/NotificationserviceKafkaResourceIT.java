...
import com.app.notificationservice.config.KafkaSseConsumer;
import com.app.notificationservice.config.KafkaSseProducer;
...
class NotificationserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/notificationservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/notificationservice-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/notificationservice-kafka/unregister"));
                return;
            }
        }
